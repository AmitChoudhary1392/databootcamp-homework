...
data.forEach((element)=> {
    ...
    if (key=='Title'){
        ...
        cell.html(`"<a href='/getuserinputs'>${value}</a>"`)
    /* cell.innerHTML= value.link('/getuserinputs') */
    }
    else if (key=='image_url'){
        var img= document.createElement('img');
        var a=document.createElement('a')
        a.href="/getuserinputs"
        var link=cell.appendChild(a);
        img.src= value;
...
var url2 = "/api/sharebook_results";
d3.json(url2, function(response) {
    ...
    book_table.selectAll('td').on('click', function(){
       ...
       /*  var form_title=d3.getElementById('#inputTitle'); */
        ...
        d3.json(url2, function(datasheet){
